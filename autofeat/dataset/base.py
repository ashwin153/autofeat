...
class Dataset(abc.ABC):
    ...
    def features(
        ...
        *,
        ...
        """Extract features under the ``transform`` and for each of the ``filters``.

        Features are the boolean or numeric columns from tables that reduce to a single row under
        the ``transform``. A simple way to guarantee this is to always filter and aggregate the data
        by the same columns.
